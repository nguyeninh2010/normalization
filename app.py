...
if isi_file and scopus_file:
    try:
        ...
        # =========================
        # BƯỚC 4 - GIỮ LOGIC, CHỈ ĐỔI GIAO DIỆN
        # =========================
        ...
        with m1:
            st.metric("Số mapping", len(st.session_state["approved_mapping"]))
        with m2:
            st.metric("Tổng bản ghi", len(merged_final))
        with m3:
            st.metric("Có DE_ID", int((merged_final["DE_ID"] != "").sum()))
        ...
        with st.expander("📥 Tải file và hướng dẫn sử dụng", expanded=False):
            st.markdown("""
**1. Merged đầy đủ (CSV)**  
→ Dữ liệu sau khi ghép ISI + Scopus và đã chuẩn hóa. Dùng để lưu trữ hoặc phân tích tiếp.

**2. File chuẩn cho VOSviewer (CSV)**  
→ File dùng trực tiếp để import vào VOSviewer.

**3. Bảng chỉnh sửa mapping (CSV)**  
→ File bảng làm việc, gồm cả các dòng chưa chọn. Dùng để chỉnh tiếp ngoài Excel nếu cần.

**4. Mapping đã duyệt (CSV)**  
→ Danh sách các cặp Original → Suggested mà bạn đã chọn. Dùng làm thesaurus cho lần sau.
""")
